...
animal you've ever seen; if you want a better look, you need to get ahead of it.
...
Here's the same example sketch with extra, non-main-loop pipe tiles also shown:
7-F7-
.FJ|7
SJLL7
|F--J
...
object DataDefs:
  enum Exit:
    case N, S, E, W
    def isOpposite(that: Exit) = this match
      case N => that == S
      case S => that == N
      case E => that == W
      case W => that == E
...
object Parsing:
  ...
  def parseExits(lines: IndexedSeq[String]): Maze[Exits] = lines.map(_.map(_.toExits))

  def parseCoords(maze: Maze[Exits]): Maze[Coord] =
    for row <- 0 until maze.length
    yield (for
      col <- 0 until maze.length
      exits = maze(row)(col)
    yield Coord(row, col, exits))
