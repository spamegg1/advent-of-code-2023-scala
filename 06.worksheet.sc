 ...
 */
object DataDefs:
  type Time = Long; type Distance = Long
  case class Race(time: Time, distance: Distance)
...
object Parsing:
  ...
  def parseLine2(line: String) = line match // part 2
    case s"Time: $a"     => Seq(a.filterNot(_.isSpaceChar).toLong)
    case s"Distance: $a" => Seq(a.filterNot(_.isSpaceChar).toLong)

  def parseData(lines: Seq[String])(parser: String => Seq[Long]): Seq[Race] =
    val (times, distances) = (parser(lines(0)), parser(lines(1)))
...
object Solving:
  ...
  def waysToWin(race: Race): Long =
    ...
    val (x1, x2) = (t / 2 - disc(t, d) / 2, t / 2 + disc(t, d) / 2)
    // are x1 and x2 already integers?
    val res1 = if x1.ceil == x1 then x1.ceil + 1.0 else x1.ceil
