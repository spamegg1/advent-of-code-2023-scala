...
object DataDefs:
  ...
  enum Op:
    case Remove(l: Label)
    case Update(l: Label, power: Power)
    def label = this match
      case Remove(l)    => l
      case Update(l, _) => l

  case class Lens(label: Label, power: Power)

object Parsing:
  import DataDefs.*, Op.*
  def parseFile(file: String) = file.split(",").toList
  def parseStep(step: String): Op = step match
    case s"$label=$power" => Update(label, power.toInt)
    case s"$label-"       => Remove(label)

object Solving:
  import DataDefs.*, Op.*, collection.mutable.{Map, Queue}
  private def hash(label: Label): Box = label.foldLeft(0): (current, char) =>
    (current + char.toInt) * 17 % 256

  private val HASHMAP = (for box <- 0 until 256 yield box -> Queue[Lens]()).toMap

  private def populate(ops: List[Op]): Unit =
    for op <- ops do
      val box = hash(op.label)
      val queue = HASHMAP(box)
      val index = queue.indexWhere(_.label == op.label)
      (op, index) match
        case (Remove(_), -1) => // do nothing
        case (Remove(_), _)  => queue.remove(index) // this will move up other lenses
        case (Update(label, power), -1) => queue.enqueue(Lens(label, power))
        case (Update(label, power), _)  => queue.update(index, Lens(label, power))

  private def handleQueue(queue: Queue[Lens]): Power = queue.zipWithIndex
    .map((lens, index) => lens.power * (index + 1))
    .sum

  private def focusingPower: Power = HASHMAP
    .map((box, queue) => (box + 1) * handleQueue(queue))
    .sum

  def solve1(file: String): Box = Parsing
    .parseFile(file)
    .map(hash(_))
    .sum

  def solve2(file: String): Power =
    val ops = Parsing.parseFile(file).map(Parsing.parseStep(_))
    populate(ops)
    focusingPower
